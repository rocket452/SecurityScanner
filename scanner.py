...
def resolve_targets(args):
    ...
    if args.fetch_scope:
        ...
        fetcher = HackerOneAPIScopeFetcher(
            username=args.h1_username,
            api_token=args.h1_token
        )
...
Examples:
  ...
  # HackerOne with authentication (private programs)
...
def generate_reports(scan_results, args, target=None):
    """
    Generate and save all reports.
    Prints to console and optionally saves to file.
    """
    # Use provided target or fall back to args.target
    if target is None:
        target = args.target
